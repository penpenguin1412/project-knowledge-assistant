...
**当前验证状态：**本地功能测试通过，30 题原文检索基线已生成；真实模型端到端评测尚未运行。此仓库不以离线检索、规则提取或测试替身冒充大模型结果。GitHub 上传与线上部署是独立步骤，不能从本地可运行推断已经发布。
...
默认 **不调用任何模型，不需要 API 密钥**。本应用使用兼容 `/v1/chat/completions` 的接口，模型须支持 `response_format: {"type":"json_object"}`、`temperature` 与 `max_tokens`。不是所有提供商都兼容；不兼容时明确报错，不会静默切换模型。
...
模型请求没有自动重试；单次输出预算为 1800 tokens，网络连接和每次读取超时均为 45 秒。前端等待 65 秒后提示超时；超时并不能证明提供商没有计费，先查运行记录再决定是否重试。问答只发送前 5 个片段和问题；会议提取发送用户提交的会议全文。状态中的“已启用”只表示配置完整，不代表连接测试通过。
...
- BM25 对同义改写、复杂推理、跨段证据与冲突识别有限。模型拒答依赖提示词与模型判断；引用校验不能完全防止语义幻觉或提示注入。本应用不给模型工具、文件系统、数据库或执行权限。
